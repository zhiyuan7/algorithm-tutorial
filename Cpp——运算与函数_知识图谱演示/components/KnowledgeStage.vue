...
import ExpressionMorph from './ExpressionMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps['judgment-expression'] : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <ExpressionMorph v-if="scene.map === 'morph'" :key="scene.id" />
    ...
    <div class="stage-corner-label">Cpp · 运算与函数</div>
  </main>
</template>
