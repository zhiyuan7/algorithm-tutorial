...
function boundsFor(nodes: LayoutNode[]) {
  const safe = nodes.length ? nodes : [{ x: 800, y: 450, width: 1, height: 1 }] as LayoutNode[]
  const left = Math.min(...safe.map(node => node.x - node.width / 2))
  const right = Math.max(...safe.map(node => node.x + node.width / 2))
  const top = Math.min(...safe.map(node => node.y - node.height / 2))
  const bottom = Math.max(...safe.map(node => node.y + node.height / 2))
...
export function useCamera(scene: Ref<TourScene>, nodes: Ref<LayoutNode[]>) {
  return computed(() => {
    ...
    const viewY = VIEW_HEIGHT / 2 + (current.mode === 'detail' ? 40 : 25)
    return {
