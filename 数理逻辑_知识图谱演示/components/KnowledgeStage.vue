...
import QuantifierMorph from './QuantifierMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps['quantifier-dependency'] : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <QuantifierMorph v-if="scene.map === 'morph'" :key="scene.id" />
