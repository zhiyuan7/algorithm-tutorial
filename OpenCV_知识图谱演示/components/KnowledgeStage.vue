...
import PipelineMorph from './PipelineMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps['processing-pipeline'] : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <PipelineMorph v-if="scene.map === 'morph'" :key="scene.id" />
    ...
    <div class="stage-corner-label">OpenCV · Knowledge Map</div>
  </main>
</template>
