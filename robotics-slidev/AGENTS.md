...
9. 两张知识图谱的数据保持独立。`KinematicsMorph.vue` 只是过渡镜头，不得把两张图连成一张大图。
10. 修改后必须运行 `typecheck`、`build`和浏览器 Visual QA。
11. 检查详细场景、收缩场景、跨分支镜头、morph 与最终 fitAll，不只检查封面。
12. 使用 Node.js 22.12 或更新版本；Linux 可直接运行 pnpm，Windows 可用 `scripts/run.ps1`。
