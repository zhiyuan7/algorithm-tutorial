...
const props = defineProps<{
  ...
  visible: boolean
  active: boolean
  mapLayout: 'diamond' | 'tree'
}>()

const path = computed(() => {
  const { source, target } = props.edge
  if (props.mapLayout === 'diamond' && props.edge.type !== 'hierarchy') {
    const controls: Record<string, [number, number]> = {
      'r-reference-rotation': [1215, 145],
      'r-rotation-transform': [1215, 755],
      'r-transform-compact': [385, 755],
    }
