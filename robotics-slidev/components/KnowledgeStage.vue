...
import KinematicsMorph from './KinematicsMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps.kinematics : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <KinematicsMorph v-if="scene.map === 'morph'" :key="scene.id" />
    ...
    <div class="stage-corner-label">简明机器人学 · Knowledge Map</div>
  </main>
</template>
