...
import RuntimeMorph from './RuntimeMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps.execution : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <RuntimeMorph v-if="scene.map === 'morph'" :phase="scene.morphPhase ?? 0" />
    ...
    <div class="stage-corner-label">ROS 2 · Knowledge Map</div>
  </main>
</template>
