...
整套演示沿用《总纲》项目的 `Minimal / Academic / Technical / Cinematic` 风格。信息层级优先于装饰，镜头运动服务于空间记忆与知识关系。

颜色与配置直接派生自 `course-overview-slidev`。主背景采用暖黄 `#D6BF88`，金黄 `#D7B662` 只用于小面积辅助，保证两套课件具有一致的视觉身份。
...
三张知识图谱在数据上保持独立。`SetTheoryMorph.vue` 只承担两次中间镜头：

- 外延、分离、构造公理重组为有序对、关系/等价类、自然数/数系。
- 有序对/函数、可数数系、实数/幂集重组为双射、可数无限、不可数与更大无限。
- 来源图谱的边退场后，目标标签与目标关系才进入。

不得把三张图谱连成一张超大树；过渡映射只表达叙事上的意义保留，不宣称概念严格等价。
