...
import ComputabilityComplexityMorph from './ComputabilityComplexityMorph.vue'
import ComplexityUniversalityMorph from './ComplexityUniversalityMorph.vue'
...
const activeMapId = computed(() => {
  if (scene.value.map === 'morph-a-b') return 'complexity'
  if (scene.value.map === 'morph-b-c') return 'universality'
  return scene.value.map
})
const activeMap = computed(() => knowledgeMaps[activeMapId.value])
...
<template>
  <main class="knowledge-stage">
    ...
    <ComplexityUniversalityMorph v-else-if="scene.map === 'morph-b-c'" :key="scene.id" />
