...
import ControlMorph from './ControlMorph.vue'
...
const activeMap = computed(() => scene.value.map === 'morph' ? knowledgeMaps['algorithm-assembly'] : knowledgeMaps[scene.value.map])
...
<template>
  <main class="knowledge-stage">
    ...
    <ControlMorph v-if="scene.map === 'morph'" :key="scene.id" />
    ...
    <div class="stage-corner-label">Cpp · 执行顺序</div>
  </main>
</template>
